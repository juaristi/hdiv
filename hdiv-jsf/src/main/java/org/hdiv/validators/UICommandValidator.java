...
public class UICommandValidator implements ComponentValidator {
	...
	public ValidationError validate(UIComponent component) {
		...
		UIData tabla = UtilsJsf.findParentUIData(command);
		...
		for (UIComponent childComp : component.getChildren()) {
			if (childComp instanceof UIParameter) {
				...
				ValidationError error = processParam(param, rowIndex);
				if (error != null) {
					return error;
				}
			}
	 ...
	 */
	private ValidationError processParam(UIParameter parametro, int rowIndex) {

		UIParameterExtension param = (UIParameterExtension) parametro;
		FacesContext fContext = FacesContext.getCurrentInstance();

		UIComponent parent = parametro.getParent();
		String parentClientId = parent.getClientId(fContext);

		Map<String, String> requestMap = fContext.getExternalContext().getRequestParameterMap();
		...
		if (requestValue == null) {
			...
			error.setErrorComponent(param.getClientId(fContext));
			return error;
