...
public class JsfValidatorHelper extends ValidatorHelperRequest {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.hdiv.filter.AbstractValidatorHelper#preValidate(javax.servlet.http.HttpServletRequest, java.lang.String)
	 */
	...
	protected Boolean preValidate(HttpServletRequest request, String target) {
		...
		if (isViewState) {
			...
			if (log.isDebugEnabled()) {
				log.debug("is view state");
			}

			if (this.getHdivConfig().isStartPage(this.getTargetWithoutContextPath(request, target))) {
				// It is an init page
				if (log.isDebugEnabled()) {
					log.debug("is start page");
				}
				return Boolean.TRUE;
			}
		...
		// Delegate to AbstractValidatorHelper
		return null;
